...
def compra_animales():
    ...
    match opcion:
        case 1 | 2| 3 | 4 | 5:
            ...
            val_cant = True
            while val_cant:
                cantidad = input(f"Cuantas {animal}s quieres comprar?: ")
                if cantidad.isdigit():
            ...
            print(f"El total a pagar seria {total} G")
            
            if saldo_user >= total:
                saldo_user -= total
                inventario[animal]+= cantidad
                print("Tu compra ha sido exitosa!")
            else:
                print("No tienes saldo suficiente para este compra.\n")
            mostrar_tienda()
            compra_animales()
        case 6:
            animales_en_stock = {animal: cantidad for animal, cantidad in inventario.items() if cantidad > 0}
            if len(animales_en_stock) == 0:
                print("No tienes animales para vender.")
                mostrar_tienda()
                compra_animales()
                return
            else:
            ...
            print(f"\nVendiste {cantidad_v} {animal_elegido}(s) por {total_venta} G.")
            print(f"Nuevo saldo: {saldo_user} G")
            mostrar_tienda()
            compra_animales()
        case 7:
